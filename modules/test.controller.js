...
const outputObj = (v) => { return { solution: util.inspect(v, false, null, true)} }
...
const milesconversion = (x) => {

  const dist = parseFloat((x / 1610).toFixed(1))
  console.log(dist)
...
const solution = () => {
  ...
  /* 5 miles */
  const maxRouteLength = 8046 + (8046 * 0.3); // meters     10460meters
  /* Parent Object */

  console.log(distances[0].elements)
  console.log("-------------------------------------------")
  console.log(distances[1].elements)
  console.log("-------------------------------------------")
  const calculations = {
  ...
  while (calculations.customersRemaining > 0) {
    if (calculations.prevDistance === 0) {
      ...
      for (let i = 0; i < calculations.nod; i += 1) {
        ...
        // console.log(calculations.dosfec[0].allocated)
        // console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        console.log("*************************************************************")
        console.log(!calculations.dosfec[i].allocated)  //allocation wasnt there means its oposite = true
      ...
      routeObject.path = `${milesconversion(calculations.dosfec[closest].distance.value)} mi from Store to ${customers[closest]}.`
      ...
      console.log(calculations.customersRemaining + " : customers remaining")

      calculations.lastAllocated = closest;
      destinationChecked.check[calculations.lastAllocated] = true    // lastallocated is done
     ...
     */
      ...
      if (
        ...
        // SOLUTION
        console.log(calculations.prevDistance + " tESt")


        console.log(calculations.lastAllocated + " testing")


        let closestoLastAllocatedDistance = distances[calculations.lastAllocated + 1].elements[0].distance.value
        let closesttoLastAllocatedCustomer = 0

        for (let i = 0; i < calculations.nod; i++) {

          console.log(distances[calculations.lastAllocated + 1].elements[i].distance.value)

          if (distances[calculations.lastAllocated + 1].elements[i].distance.value <= closestoLastAllocatedDistance
            && (calculations.lastAllocated !== i) && (destinationChecked.check[i] !== true)) {
            console.log("Hi testing again for A last")

            closestoLastAllocatedDistance = distances[calculations.lastAllocated + 1].elements[i].distance.value
            closesttoLastAllocatedCustomer = i

          }
        }
        let x = customers[calculations.lastAllocated]
        let y = customers[closesttoLastAllocatedCustomer]
        console.log(closesttoLastAllocatedCustomer + " y value")

        if ((calculations.prevDistance + closestoLastAllocatedDistance) < maxRouteLength
          && (destinationChecked.check[closesttoLastAllocatedCustomer] !== true)) {
          const routeObject = {};
          /* Set the customer to the route */
          routeObject.customer = y;
          /* Get the charges for the customer using calculate charges method */
          routeObject.deliveryCharges = calculateCharges(closestoLastAllocatedDistance);
          routeObject.path = `${milesconversion(closestoLastAllocatedDistance) === 0 ? " 1 ft" : milesconversion(closestoLastAllocatedDistance) + " mi"} from ${x} to  ${y} "  `
          destinationChecked.check[closesttoLastAllocatedCustomer] = true
          calculations.currentRoute.push(routeObject);

          //  destinationChecked.check[y] =true
          calculations.prevDistance = calculations.prevDistance + closestoLastAllocatedDistance

          calculations.lastAllocated = closesttoLastAllocatedCustomer;



          console.log(`${closestoLastAllocatedDistance} mi from ${x} to  ${y}`)
          console.log(destinationChecked)
          console.log((calculations.prevDistance + closestoLastAllocatedDistance) + " :total lenth"
            + maxRouteLength + " : maxRouteLength")


        }
        //else if ((calculations.prevDistance + closestoLastAllocatedDistance) > maxRouteLength) {
        //   console.log("Came to else statement where max length maybe greater then x")
        ...
        if (destinationChecked.check.indexOf(false) !== -1 && calculations.customersRemaining === 1) {
          let notChecked = destinationChecked.check.indexOf(false)

          calculations.routeNumber += 1;
          // Push the route calculated to the all deliveryRoutes 
          calculations.deliveryRoutes.push({ totalLength: `${milesconversion(calculations.prevDistance)} miles from Store.`, route: calculations.currentRoute });
          // Set the total distance of the route to be zero
          calculations.prevDistance = 0;
          // set the current route be empty
          calculations.currentRoute = [];
          ...
          const routeObject = {};
          /* Set the customer to the route */
          routeObject.customer = customers[notChecked];
          /* Get the charges for the customer using calculate charges method */
          routeObject.deliveryCharges = calculateCharges(calculations.dosfec[notChecked].distance.value);
          routeObject.path = `${milesconversion(calculations.dosfec[notChecked].distance.value)} miles from Store to ${customers[notChecked]}.`
          calculations.currentRoute.push(routeObject);

          calculations.prevDistance = calculations.dosfec[notChecked].distance.value
        ...
        console.log(calculations.customersRemaining + " : customers remaining down")
        //if(destinationChecked.check.indexOf(false) !== -1 )
      } else {
